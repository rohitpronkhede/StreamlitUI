...
def main():
    ...
    if st.session_state.is_signed_in:
        if st.query_params.get("action") == "chat":
            user_role = st.session_state.user_role
            if user_role == "admin":
                # Redirect to admin chat page
                st.write("Admin Chat Interface")
            elif user_role == "normal":
                # Redirect to normal user chat page
                st.write("Normal User Chat Interface")
        else:
